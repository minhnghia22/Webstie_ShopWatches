...
namespace ShopWatches.Controllers
{
    public class CartsController : Controller
    {
        ...
        public ActionResult CheckCode(String code)
        {
            ...
            Voucher v = db.Voucher.Where(x => x.code == code && x.status == "false" && x.exprityDate < DateTime.Today).FirstOrDefault();
            if(v != null)
            {
                Session["voucher"] = v;
                v.status = "true";
                db.Entry(v).State = EntityState.Modified;//danh dau la co sua doi thuoc tinh cua thuc the
